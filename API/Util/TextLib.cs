...
public static class TextLib {
    ...
    public static ThemeColor GetColor(float val, int threshold, bool isPositive = true) =>
        val > threshold ? GetIncColor(isPositive) : val < threshold ? GetDecColor(isPositive) : ThemeColor.Imp;

    /// <returns>
    /// The color that should be used when the value is increased beyond normal, depending on if it is desirable
    /// </returns>
    public static ThemeColor GetIncColor(bool isPositive) => isPositive ? ThemeColor.Pos : ThemeColor.Neg;

    /// <returns>
    /// The color that should be used when the value is decreased below normal, depending on if it is desirable
    /// </returns>
    public static ThemeColor GetDecColor(bool isPositive) => isPositive ? ThemeColor.Neg : ThemeColor.Pos;
    ...
    public static string FormatStat(int stat, int statDefault) =>
        stat.Format(stat > statDefault ? ThemeColor.Pos : stat < statDefault ? ThemeColor.Neg : ThemeColor.Imp, false);
}